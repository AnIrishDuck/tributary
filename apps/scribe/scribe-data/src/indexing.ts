...
interface NoteSlugQueryRow {
  block_uuid: string;
  slug: string;
  body: string;
}

interface NoteSlugDetailRow extends NoteSlugQueryRow {
  insert_datetime: string;
  collection_id: string | null;
  block_type: string | null;
}

interface BlockSlugQueryRow extends NoteSlugQueryRow {
  block_type: string | null;
}
...
export async function indexSlugs(
  ...
  await localDb.transaction(async (tx: QueryableTransaction) => {
...
export async function getNoteSlugByUuid(
  ...
  const row = result.rows[0] as { block_uuid: string; slug: string; body: string; block_type: string | null }
...
export async function getAllNotesWithTitles(db: TributaryLocal): Promise<NoteSlugRow[]> {
  ...
  return (result.rows as NoteSlugDetailRow[] || []).map((row) => {
    const blockType = row.block_type || 'scribe/markdown'
    return {
      block_uuid: row.block_uuid,
      slug: row.slug,
...
export async function getNotesBySlugInCollection(
  ...
  return (result.rows as BlockSlugQueryRow[] || []).map((row) => ({
    block_uuid: row.block_uuid,
    slug: row.slug,
