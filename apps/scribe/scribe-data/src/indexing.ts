...
interface BlockListingRow extends BlockRow {
  insert_datetime: string;
...
export async function getNotesBySlug(
  ...
  return (result.rows || []).map((row: { block_uuid: string; slug: string; body: string }) => ({
    block_uuid: row.block_uuid,
    slug: row.slug,
...
export async function getNotesByTag(
  ...
  return (result.rows || []).map((row: { block_uuid: string }) => row.block_uuid)
}
...
export async function getAllNotesWithTitles(db: TributaryLocal): Promise<NoteSlugRow[]> {
  ...
  return (result.rows || []).map((row: BlockListingRow) => {
    const blockType = row.block_type || 'scribe/markdown'
    return {
      block_uuid: row.block_uuid,
      slug: row.slug,
...
export async function getNotesBySlugInCollection(
  ...
  return (result.rows || []).map((row: BlockRow) => ({
    block_uuid: row.block_uuid,
    slug: row.slug,
