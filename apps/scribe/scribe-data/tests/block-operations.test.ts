...
import { BlockUuid, VersionUuid, Block } from '../src/types.js'
...
describe('scribe-data migrations and operations', () => {
  ...
  test('should create block table with correct schema', async () => {
    ...
    const result = await db.selectFrom('block').selectAll().executeTakeFirst()
  ...
  test('should insert and retrieve a block', async () => {
    ...
    await db.insertInto('block')
      .values({
        ...
        prior_version_uuid: block.prior_version_uuid,
        insert_datetime: block.insert_datetime,
    ...
    // Retrieve the block
    const result = await db.selectFrom('block')
      .selectAll()
  ...
  test('should allow multiple versions of the same block', async () => {
    ...
    const blockVersion2 = {
      ...
      insert_datetime: new Date(now.getTime() + 1000), // 1 second later
      body: 'Second version'
    }
    ...
    const results = await db.selectFrom('block')
      .selectAll()
  ...
  test('should query all versions of a block', async () => {
    ...
    const blocks = [
      ...
      },
      {
        ...
        insert_datetime: new Date(now.getTime() + 1000),
        inserter: 'user-1',
    ...
    const block1Versions = await db.selectFrom('block')
      .selectAll()
    ...
    const block2Versions = await db.selectFrom('block')
      .selectAll()
