...
import { indexCollectionSlugs, getNotesInCollectionWithSlugs, indexAll, getNotesBySlugInCollection, extractTitleFromMarkdown } from '../src/indexing.js'
...
describe('Collection Operations', () => {
  ...
  describe('Collision Detection', () => {
    ...
    test('checkMoveCollision detects collision with existing collection', async () => {
      ...
      const collision = await checkMoveCollision(
        localDb, 'recipes', null, library.collection_uuid, noteToMove.block_uuid
      )
      expect(collision).toBe(true)
    })
    ...
    test('checkMoveCollision excludes the entity itself from collision check', async () => {
      ...
      const collision = await checkMoveCollision(
        localDb, 'my-note', null, library.collection_uuid, note.block_uuid
      )
      expect(collision).toBe(false)
    })
    ...
    test('checkMoveCollision excludes collection itself from collision check', async () => {
      ...
      const collision = await checkMoveCollision(
        localDb, 'recipes', null, library.collection_uuid, collection.collection_uuid
      )
      expect(collision).toBe(false)
    })
  })
