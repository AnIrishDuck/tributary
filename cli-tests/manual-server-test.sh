...
# Manual test to bypass the client and test server directly
...
# Compute hash of the data (we'll simulate what the server should do)
# In Node.js for consistency
HASH=$(node -e "
...
# Generate a signature (this is harder to do in bash, so we'll use Node.js)
SIGNATURE=$(node -e "
