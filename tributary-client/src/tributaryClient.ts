...
export class TributaryClient {
  ...
  async addWriteKey(appId: string, key: string | Uint8Array): Promise<TributaryStream> {
    ...
    // Eagerly persist stream metadata so list() can return it
    await stream.ensurePersisted();
    info(`[addWriteKey] ensurePersisted done at ${(performance.now() - t0).toFixed(0)}ms`);
